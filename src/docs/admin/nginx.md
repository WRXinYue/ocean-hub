# Nginx configuration
1. Create `/etc/nginx/conf.d/ocean.conf` or `/etc/nginx/sites-available/ocean.conf` and copy the following example to the file.\
   (The file name does not have to be "ocean".)
2. Edit as follows:
...
3. If you create `/etc/nginx/sites-available/ocean.conf`, create symlink as `/etc/nginx/sites-enabled/ocean.conf`.\
