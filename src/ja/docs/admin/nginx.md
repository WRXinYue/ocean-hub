# Nginxの設定
1. `/etc/nginx/conf.d/ocean.conf`もしくは`/etc/nginx/sites-available/ocean.conf`を作成し、下の設定例をコピーします。\
   （ファイル名はoceanでなくても構いません。）
2. 次のように編集します。
...
3. `/etc/nginx/sites-available/ocean.conf`を作成した場合は、`/etc/nginx/sites-enabled/ocean.conf`としてシンボリックリンクを作成します。\
